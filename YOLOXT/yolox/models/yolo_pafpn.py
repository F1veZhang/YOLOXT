...
class YOLOPAFPN(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed3 = PatchEmbed(
            patch_size=1, in_c=128, embed_dim=128,
            norm_layer=nn.LayerNorm)
        self.ffim1 = Fusion_1()
        self.ffim2 = Fusion_2()
    ...
    def forward(self, input):
        ...
        b, c, H, W = pan_out2.shape
        pan_out2, H, W = self.patch_embed3(pan_out2)  # tuple
        attn_mask = self.create_mask(x=pan_out2, H=H, W=W).to(pan_out2.device)
        for blk in self.swin3:
            blk.H, blk.W = H, W
            # x0=torch.tensor([x0.detach().numpy() for i in x0])
            pan_out2 = blk(pan_out2, attn_mask)
        pan_out2 = pan_out2.permute(0, 2, 1).view(b, c, H, W)
